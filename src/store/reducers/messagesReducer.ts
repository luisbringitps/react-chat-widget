...
const messagesReducer = {
  [ADD_NEW_USER_MESSAGE]: (state: MessagesState, { text, footer, id }) =>
    ({ ...state, messages: [...state.messages, createNewMessage(text, MESSAGE_SENDER.CLIENT, footer, id)]}),

  [ADD_NEW_RESPONSE_MESSAGE]: (state: MessagesState, { text, footer, id }) =>
    ({ ...state, messages: [...state.messages, createNewMessage(text, MESSAGE_SENDER.RESPONSE, footer, id)], badgeCount: state.badgeCount + 1 }),
